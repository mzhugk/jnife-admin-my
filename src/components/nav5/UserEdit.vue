  ...
  <section id="NewUser">
    ...
    <el-dialog title="KPI设置" :visible.sync="dialogFormVisible">
      ...
      <div  style="text-align: center">
        <el-button type="primary" @click="submitKPIForm('KPIForm')" :status="status">确 定</el-button>
...
//          }
    ...
    methods: {
      submitForm(formName) {
        let that=this;
        this.form.id=sessionStorage.getItem("user_id");
//        this.$refs[formName].validate((valid) => {
//          if (valid) {
            this.$ajax.post("https://www.jnife.com/index.php/admin/admin/admin_edit",qs.stringify(this.form))
              .then(function (res) {
                if(res.data.code==100000){
                  that.$message.success('恭喜，用户编辑成功');
                  that.$router.push("UserManage");
                  that.$store.commit('head_icon')//同步右上角的用户信息
                }
              }).catch(function () {
        ...
        if(this.status==0){
          this.$refs[formName].validate((valid) => {
            if (valid) {
              ...
              KPIFormCopy.admin_id=sessionStorage.getItem("user_id");
              this.$ajax.post("https://www.jnife.com/index.php/admin/admin/addKpiForEdit",qs.stringify(KPIFormCopy))
                .then(function (res) {
                  if(res.data.code==100000){
          ...
          this.$refs[formName].validate((valid) => {
            if (valid) {
              let {...KPIFormCopy}=this.KPIForm;
              KPIFormCopy.total=this.total;
              KPIFormCopy.kpi_id=this.kpi_id;
              KPIFormCopy.admin_id=sessionStorage.getItem("user_id");
              this.$ajax.post("https://www.jnife.com/admin/admin/editKpi",qs.stringify(KPIFormCopy))
                .then(function (res) {
                  if(res.data.code==100000){
                    that.$message.success('恭喜，KPI编辑成功');
//                    let obj={};
//                    obj.year=that.KPIForm.year;
//                    obj.jan=that.KPIForm.jan;
//                    obj.feb=that.KPIForm.feb;
...
//                    that.tableData.push(obj);
      ...
      edit(row){
        ...
        this.dialogFormVisible=true;
        this.kpi_id=row.id;
        this.$ajax.get("https://www.jnife.com/admin/admin/editKpi?kpi_id="+this.kpi_id)
          .then(function (res) {
            if(res.data.code==100000){
              that.KPIForm=res.data.kpi;
            }
          })
          .catch(function (err) {
...
//					type: 'warning'
          ...
          this.$ajax.get("https://www.jnife.com/admin/admin/deleteKpi?kpi_id="+this.kpi_id+"&admin_id="+admin_id)
            .then(function (res) {
              if(res.data.code==100000){
                that.$message.success("该KPI删除成功");
                that.tableData=[];
                that.tableData=res.data.kpi;
              }
            })
    ...
    created:function () {
      ...
      this.$ajax.get("https://www.jnife.com/index.php/admin/admin/admin_edit?id="+id)
        .then(function (res) {
          if(res.data.code==100000){
              that.form=res.data.data.data;
              that.tableData=res.data.data.kpi;
          }
        })
        .catch(function (err) {
